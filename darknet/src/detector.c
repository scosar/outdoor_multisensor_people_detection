    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile)
{
    ...
    // SC: Make output directory
    char out_dir[256];
    memcpy( out_dir, &outfile[9], strlen(outfile)-9 );
    out_dir[strlen(outfile)-9] = '\0';
    
    printf("Output: %s \n", out_dir);
    int a = mkdir(out_dir,S_IRWXU | S_IRWXG | S_IRWXO );//SC
    if (a!=0 && errno!=17)  //SC
        printf("Error (%d) %s: %s \n",errno,strerror(errno), out_dir);  //SC
    ...
    while (1) {
        ...
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
        draw_detections_v3_onlyperson(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output); // SC
        sprintf(filename_out,"%s/predictions_%d",out_dir,fileNo); //SC
        save_image(im, filename_out); //SC
        fileNo++; //SC
        if (!dont_show) {
