...
def placeholder_inputs(batch_size, num_point):
    pointclouds_pl = tf.placeholder(tf.float32,
                                     shape=(batch_size, num_point, 3)) # SC: changed 9 to 3
...
if __name__ == "__main__":
    with tf.Graph().as_default():
        a = tf.placeholder(tf.float32, shape=(32,4096,3)) # SC: changed 9 to 3
        ...
        with tf.Session() as sess:
            ...
            for i in range(100):
                print(i)
                sess.run(net, feed_dict={a:np.random.rand(32,4096,3)}) # SC: changed 9 to 3
